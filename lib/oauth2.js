...
This allows users that have Google accounts (or Google for Work accounts) to log in
to the application. [Read more about OpenID Connect](https://developers.google.com/accounts/docs/OpenIDConnect)

It performs the [OAuth2 Web Server Flow](https://developers.google.com/accounts/docs/OAuth2WebServer)
...
module.exports = function(config, actions) {
  ...
  router.get('/oauth2callback', function(req, res) {
    ...
    getClient().getToken(req.query.code, function(err, tokens) {
      ...
      getUserProfile(client, function(err, profile) {
        ...
        actions.create(profile.displayName, "Logged in");
  ...
  */
  router.get('/oauth2/logout', function(req, res) {
    actions.create(req.session.profile.displayName, "Logged out");
  ...
  */
  router.get('/oauth2/user_info', authRequired, function(req, res) {
    getUserProfile(req.oauth2client, function(err, profile) {
      if (err) return res.status('500').send(err.message);
      res.json(profile);
    });
  });
  ...
  /*
    Shows information about the user's id_token provided by Google.
  */
  router.get('/oauth2/token_info', authRequired, function(req, res) {
    /* jshint camelcase: false */
    var client = getClient();
    client.verifyIdToken(req.session.oauth2tokens.id_token, null, function(err, login) {
      if (err) return res.status('500').send(err.message);
